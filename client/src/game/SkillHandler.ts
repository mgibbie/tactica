...
export class SkillHandler {
    ...
    public confirmSkill(
        ...
        if (currentSkill?.id === 'mistwalk') {
            ...
            // Use the imported globalTileEffectManager to check tile effects
            const effects: any[] = globalTileEffectManager.getEffectsAtPosition(destination) || [];
            // Accept tiles that are mist-tile and either have matching appliedBy or no appliedBy metadata
            const isValid = effects.some((e: any) => e.effectId === 'mist-tile');
            if (!isValid) {
        ...
        // Special handling for Misticism - place mist tiles at target (exact range 3 cardinal) and its 4 cardinals
        if (currentSkill?.id === 'misticism') {
            const casterPos = getUnitPosition ? getUnitPosition(selectedUnit) : null;
            if (!casterPos) return null;
            const dx = Math.abs(targetPosition.x - casterPos.x);
            const dy = Math.abs(targetPosition.y - casterPos.y);
            const manhattan = dx + dy;
            if (!(manhattan === 3 && (dx === 0 || dy === 0))) return null;
            const tiles = [
                { x: targetPosition.x, y: targetPosition.y },
        ...
        affectedUnits.forEach((unit: Unit) => {
            if (currentSkill?.id === 'universal-whisper' || currentSkill?.id === 'healing-circle' || currentSkill?.id === 'bandage' || currentSkill?.id === 'finger-of-god') {
                ...
                // Update visuals for healed unit and show modifier consumption (Blessed/Cursed)
                const gameSceneInstance = (window as any).GAME_SCENE_INSTANCE;
                if (gameSceneInstance && gameSceneInstance.unitRenderer) {
            ...
            // Base heal equals skill damage; apply Faith/Doubt on healer and Blessed/Cursed on receivers
            const baseHeal = selectedUnit.skillDamage;
            const performHeal = ModifierService.processSkillHealPerformModifiers(selectedUnit, baseHeal);
        ...
        // Ensure healer-side modifier consumption (Faith/Doubt) is visually reflected
        if (currentSkill?.id === 'universal-whisper' || currentSkill?.id === 'healing-circle' || currentSkill?.id === 'bandage' || currentSkill?.id === 'finger-of-god') {
            const gameSceneInstance = (window as any).GAME_SCENE_INSTANCE;
            if (gameSceneInstance && gameSceneInstance.unitRenderer) {
                gameSceneInstance.unitRenderer.updateUnitModifiers(selectedUnit);
