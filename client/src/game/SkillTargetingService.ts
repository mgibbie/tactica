...
export class SkillTargetingService {
    ...
    public handleSkillTargetSelection(
        ...
        if (skill?.targetingType === 'dual-rotational') {
            ...
            // Special handling for skills that don't need rotation (like Exhaust, Jeer, Distraction, Hype Up, Inspire Violence, Mirror Aegis, Steady Beat, Switcheroo, Star's Blessing, Toxic King, Psyche Break, Flare Up, Cauterize)
            if (skill.id === 'exhaust' || skill.id === 'jeer' || skill.id === 'distraction' || skill.id === 'hype-up' || skill.id === 'inspire-violence' || skill.id === 'mirror-aegis' || skill.id === 'steady-beat' || skill.id === 'switcheroo' || skill.id === 'smoke-grenade' || skill.id === 'stars-blessing' || skill.id === 'toxic-king' || skill.id === 'psyche-break' || skill.id === 'flare-up' || skill.id === 'cauterize') {
