...
export class SkillTargetingService {
    ...
    public setupSkillTargeting(
        ...
        if (skill.targetingType === 'non-rotational' && (skill.id === 'blazing-knuckle' || skill.id === 'healing-circle' || skill.id === 'flash-of-sun' || skill.id === 'tidal-lock' || skill.id === 'outburst' || skill.id === 'symphony')) {
            ...
            uiManager.showActionSkipButton(onSkip);
        } else if (skill.id === 'box-drop' || skill.id === 'create-turret' || skill.id === 'deployable-spring' || skill.id === 'plant-the-flag' || skill.id === 'barricade' || skill.id === 'bomb-drop') {
            // Special handling for placement skills
            const skillRange = (skill.id === 'deployable-spring') ? 2 : (skill.id === 'plant-the-flag' ? 1 : 4);
            console.log(`${skill.id === 'box-drop' ? '📦' : (skill.id === 'create-turret' ? '🛡️' : (skill.id === 'plant-the-flag' ? '🏴' : (skill.id === 'barricade' ? '🧱' : (skill.id === 'bomb-drop' ? '💣' : '🌀'))))} ${skill.name} - showing valid empty tiles within range ${skillRange}`);
            const validTargets: Position[] = [];
            // Build occupancy map
            ...
            } else if (skill.id === 'gift-of-the-void') {
                skillRange = 2; // Gift of the Void: range 2 (ally only)
            } else if (skill.id === 'hired-help') {
                skillRange = 1; // Hired Help: exactly range 1
            ...
            } else if (skill.id === 'mistwalk') {
                skillRange = 12; // Mistwalk: we'll filter to creator's mist tiles; allow full-map selection
            }
            ...
            if (skill.id === 'stars-blessing' || skill.id === 'aethers-grace' || skill.id === 'exhaust' || skill.id === 'jeer' || skill.id === 'hype-up' || skill.id === 'inspire-violence' || skill.id === 'mirror-aegis' || skill.id === 'steady-beat' || skill.id === 'peace-sign' || skill.id === 'idolize' || skill.id === 'slip-counter' || skill.id === 'switcheroo' || skill.id === 'smoke-grenade' || skill.id === 'distraction' || skill.id === 'toxic-king' || skill.id === 'psyche-break' || skill.id === 'flare-up' || skill.id === 'cauterize' || skill.id === 'hired-help' || skill.id === 'gift-of-the-void' || skill.id === 'anthem' || skill.id === 'barricade' || skill.id === 'swap' || skill.id === 'rock-solid' || skill.id === 'reflect' || skill.id === 'primal-mark') {
                uiManager.showSkillConfirmCancelButtons(
                    skill.name,
                    onConfirm,
                    onCancel
    ...
    public handleSkillTargetSelection(
        ...
        if (skill?.targetingType === 'dual-rotational') {
            ...
            // For Mistwalk, confirm immediately on click (no rotate, no confirm/cancel)
            if (skill.id === 'mistwalk') {
                actionManager.setSkillTarget(skill, { x, y });
                onConfirm();
                return;
