...
export class SkillTargetingService {
    ...
    public handleSkillTargetSelection(
        ...
        if (skill?.targetingType === 'dual-rotational') {
            ...
            // Special handling for skills that don't need rotation (like Exhaust, Jeer, Distraction, Hype Up, Steady Beat, Switcheroo, Star's Blessing, Toxic King)
            if (skill.id === 'exhaust' || skill.id === 'jeer' || skill.id === 'distraction' || skill.id === 'hype-up' || skill.id === 'steady-beat' || skill.id === 'switcheroo' || skill.id === 'smoke-grenade' || skill.id === 'stars-blessing' || skill.id === 'toxic-king') {
